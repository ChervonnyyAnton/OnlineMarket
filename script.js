(function () {
  ...
  function initialize() {
    ...
    deliveryOptionElement.addEventListener(
      "change",
  ...
  function updateTotal() {
    ...
    orderButton.disabled = !(
      ...
      total > 0 &&
      (deliveryOptionElement.value === "pickup" ||
        (deliveryOptionElement.value === "delivery" && addressElement.value))
    );
  }
  ...
  function handleOrderClick() {
    ...
    if (total > 500 && deliveryOptionElement.value === "pickup") {
  ...
  function updatePastOrders() {
    ...
    orders.forEach((order) => {
      ...
      listItem.textContent = `Order placed on ${order.date} for ${
        order.total
      }. ${
        order.type === "delivery"
